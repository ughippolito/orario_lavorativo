...
// ===================== FUNZIONE BACKUP GITHUB =====================
function pushDataToGitHub() {
    // Operando dentro la cartella 'data', il file da tracciare è direttamente 'workhours.json'
    const fileToTrack = 'workhours.json'; 
    
    const now = new Date();
    const timestamp = now.toLocaleString('it-IT');
    const commitMessage = `"Auto-backup dati: ${timestamp}"`;

    // Entriamo nella cartella mappata ed eseguiamo i comandi Git
    const command = `cd ${DATA_DIR} && git pull origin main --rebase && git add ${fileToTrack} && git commit -m ${commitMessage} && git push origin main`;

    console.log(`[Git Backup] Avvio sincronizzazione su GitHub dalla cartella mappata...`);

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error(`[Git Backup] Errore esecuzione comandi Git: ${error.message}`);
            return;
        }
        if (stderr && stderr.toLowerCase().includes('error')) {
            console.error(`[Git Backup] Possibile errore: ${stderr}`);
        } else {
            console.log(`[Git Backup] Caricamento completato con successo su GitHub!`);
        }
    });
}
...
app.get('/download-manuale', (req, res) => {
    ...
    if (fs.existsSync(filePath)) {
        res.download(filePath, 'Manual_Utente_v3.pdf');
    } else {
